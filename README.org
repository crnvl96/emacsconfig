...
#+AUTHOR: Adran Carnavale
#+PROPERTY: header-args:emacs-lisp :tangle init.el :comments link

* Introduction
This is my Emacs configuration written in Org mode using literate programming.
The code blocks in this file are tangled (exported) to =init.el= for Emacs to load.

To tangle this file, open it in Emacs and run =M-x org-babel-tangle= or =C-c C-v t=.

** Setup Instructions
1. Ensure Org mode is installed and configured.
2. This file should be named =README.org= in your =~/.emacs.d/= directory.
3. The tangled output is =init.el= in the same directory.
4. For GitHub, this =README.org= serves as the project README, displaying the configuration documentation.

* Early Init
These settings are loaded early in the initialization process.

** Reducing Clutter
Redirect Emacs files to avoid cluttering =~/.emacs.d=.
...
* Package Management
Setup for package installation and use-package.
